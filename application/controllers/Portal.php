...
class Portal extends CI_Controller
{
    ...
    public function common()
    {
        ...
        if (!(isset($this->session->userdata['sid']))) {
			...
			if($urole==1){
            	...
            	$where.=" OR projectid IN (SELECT id FROM project WHERE ((accountmanager=" . $uid . ") or FIND_IN_SET( $uid, developer ))) ";
			}
    ...
    function addproject()
    {
        ...
        // primary Tech (external users)
        $data['primary_tech'] = $this->portalmodel->get_dropdownuser_list('user', 'id', $cond, 'internal_user_external_user=2','first_name','ASC');
        ...
        if ($sessionrole == 1) {
            ...
            $data['customer']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', '','name','ASC');
        } else {
        	$id          = $this->session->userdata('sid');
        	
            $where = "id IN (SELECT fk_company_id FROM customer WHERE accountmanagerid= $id AND status=1)";
            //$list  = $this->portalmodel->select_where('', '', 'customer', $where);
            //$list = $this->portalmodel->select('', '', 'customer',$where,'companyname','ASC');
            //$data['customer']  = $this->portalmodel->get_dropdown_list('customer', 'customerid', 'companyname', $where,'companyname','ASC');
            $data['customer']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', $where,'name','ASC');
        }
    ...
    function insert_project()
    {
        ...
        $data     = array(
        	...
        	'primary_tech' => $this->input->post('primary_tech'),
    ...
    function projectdetails()
    {
        ...
        $coust             = "id=" . $data['details'][0]->customerid;
        $data['company']   = $this->portalmodel->select_name('customer_company', 'name', $coust);
    ...
    function task()
    {
		...
		if(!empty($filter_created_by)){
			if($filter_created_by==1){
				// created by me
				$where.=" AND c_by=1 AND created_by='".$uid."' ";
        ...
        $customer_company_id = $prj[0]->customerid;
        $customer_company_name = $this->portalmodel->select_name('customer_company', 'name', "id='".$customer_company_id."'");
        $data['customer_company'] = $customer_company_name;
	...
	function alltask()
    {
	   ...
	   $cond="";
		...
		if(!empty($filter_created_by)){
			if($filter_created_by==1){
				// created by me
				$cond.=" AND c_by=1 AND created_by='".$uid."' ";
			}else if($filter_created_by==2){
				// created by others
		...
		$fk_customer_company_id = $this->input->get('fk_customer_company_id');
		if(!empty($fk_customer_company_id)){
			$cond.=" AND projectid IN (SELECT id FROM project WHERE customerid='".$fk_customer_company_id."')";
		}
		$data['fk_customer_company_id']=$fk_customer_company_id;
		...
		if($urole==1){
			...
			if($internal_user_external_user==1){
            	$condition_assigned_projects='1';//"(assigned_to='".$uid."' OR projectid IN (SELECT id FROM project WHERE ((accountmanager=" . $uid . ") or FIND_IN_SET( $uid, projectmanager )))) ";
            }else{
            	$condition_assigned_projects="(assigned_to='".$uid."' OR projectid IN (SELECT id FROM project WHERE ((accountmanager=" . $uid . ") or FIND_IN_SET( $uid, developer )))) ";
            }
		...
		$customer_company_id = $prj[0]->customerid;
		$customer_company_name = $this->portalmodel->select_name('customer_company', 'name', "id='".$customer_company_id."'");
		$row['customer_company'] = $customer_company_name;
        ...
        // customer dropdown.
        if ($internal_user_external_user == 1) {
            $data['customer_company']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', '','name','ASC');
        } else {
        	//$where = "id IN (SELECT fk_company_id FROM customer WHERE accountmanagerid= $id)";
            $condition_assigned_customers="id IN (SELECT customerid FROM project WHERE FIND_IN_SET( $uid, developer )) ";
            $data['customer_company']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', $condition_assigned_customers,'name','ASC');
        }
    ...
    function taskdetails()
    {
        ...
        $where8 = array();
        if($internal_user_external_user==2){
        	// get list of associated techs for the project of this task
        	$developer = $this->portalmodel->select_name('project', 'developer', "id='".$list[0]->projectid."'");
        	$where8="id IN (".$developer.")";
        }
        
        $data['assignedto'] = $this->portalmodel->get_dropdownuser_list('user', 'id', $name, $where8, 'first_name','ASC');
    ...
    function insert_task()
    {
        ...
        // now get customer id
        $customer_company_id = $this->portalmodel->select_name('project', 'customerid', "id='".$prjid."'");
        $customerid = $this->portalmodel->select_name('customer', 'customerid', "fk_company_id='".$customer_company_id."'");
...
if(isset($pmid) && !empty($pmid)){
	$arrIds[] = $pmid;
     ...
     redirect("portal/alltask?ticket=open");
    }
...
$pmid=$this->input->post('projectmanager');
		...
		// if the status of task is not started , then make it as started.
		$taskStatus = $this->portalmodel->select_name("task","status","id='".$id."'");
		if(empty($taskStatus)){
			$data_change_task_status = array(
				'status' => 1,
			);
			$updateTaskStatus = $this->portalmodel->update_query('task', $data_change_task_status, $id, 'id');
		}
...
$pmid=$this->input->post('projectmanager');
    ...
    function customerdetails()
    {
        ...
        $data['accountmanager'] = $this->portalmodel->get_dropdownuser_list('user', 'id', $cond, 'internal_user_external_user=1 AND role!=1','first_name','ASC');
        // now get customer users (for chat)
        
        $data['company'] = $this->portalmodel->get_dropdownuser_list('customer_company', 'id', 'name', '','name','ASC');
    ...
    function update_customer()
    {
        ...
        // get company name from fk_company_id
        $fk_company_id = $this->input->post('fk_company_id');
        $companyname = $this->portalmodel->select_name("customer_company","name","id='".$fk_company_id."'");
        
        //$disable_invoice = $this->input->post('disable_invoice');
        //$disable_invoice = (isset($disable_invoice))?1:0;
    ...
    function getaccountmanger()
    {
		...
		$arrAccountmanagerIds = array();
        foreach ($cust as $eachCust){
        	if(isset($eachCust->accountmanagerid) && !empty($eachCust->accountmanagerid)){
        		$arrAccountmanagerIds[]=$eachCust->accountmanagerid;
        	}
        }
        ...
        if (!empty($cust)) {
            ...
            $where = "id IN (".implode(',', $arrAccountmanagerIds).")" ;
    ...
    function syncWorkOrder(){
		...
		$contactname = $data_request['contactname'];
		...
		if(!empty($account_manager_email)){
			...
			if(empty($fk_account_manager_id) || $fk_account_manager_id=="N/A"){
		        ...
		        $data_am       = array(
		            ...
		            'role' => 2,
		            'status' => 1,
		            'internal_user_external_user' => 1	// will be internal user by default. if new created
		        );
		...
		// first check/create customer company
		$company_name = $customer_name;
		$fk_company_id = 0;
		if(!empty($customer_name)){
			$fk_company_id = $this->portalmodel->select_name('customer_company', 'id', "LOWER(name)='".strtolower($customer_name)."'");
			if(empty($fk_company_id) || $fk_company_id=="N/A"){
				// create new cutomer company
				$data       = array(
		            'name' => $customer_name
		        );
		        $result = $this->portalmodel->insert_query_('customer_company', $data);
		        if($result!="False"){
		        	$fk_company_id = $result;
		        }
			}
		}
		...
		if(!empty($customer_email)){
			$fk_customer_id = $this->portalmodel->select_name('customer', 'customerid', "emailid='".$customer_email."'");
			if(empty($fk_customer_id) || $fk_customer_id=="N/A"){
				
				// separate first name last name
				$arrName = explode(" ", $contactname);
				$arrContactName = array();
				if(count($arrName)==1){
					$arrContactName['first_name'] = $arrName[0];
					$arrContactName['last_name'] = "";
				}else if(count($arrName)==2){
					$arrContactName['first_name'] = $arrName[0];
					$arrContactName['last_name'] = $arrName[1];
				}else if(count($arrName)==3){
					$arrContactName['first_name'] = $arrName[0]." ".$arrName[1];
					$arrContactName['last_name'] = $arrName[2];
				}
		        ...
		        $data_customer = array(
		            'username' => $customerid,
		        	'fk_company_id' => $fk_company_id,
		            'first_name' => $arrContactName['first_name'],
		            'last_name' => $arrContactName['last_name'],
    ...
    function updatetechniciandropdown(){
        ...
        if(!empty($dev)){
			...
			if($internal_external==2){
	        	// its tech, so only fetch that user for the list.
	        	$where2 = "id='".$id."'";
	        }else{
	        	// regular
	        	$where2 = array();
	        }
	        $developer  = $this->portalmodel->get_dropdownuser_list('user', 'id', $cond, $where2, "first_name", "ASC");
        }else{
    ...
    public function checkduplicatecompanyname(){
        ...
        $existing_id = $this->portalmodel->select_name("customer_company","id","TRIM(LOWER(name))='".trim(strtolower($companyname))."'");
    ...
    public function exportticketreport(){
    	// load model
    	// $this->load->model('Export', 'export');
    	$this->load->model('portalmodel');
    	
    	$condition = base64_decode($_GET[base64_encode("condition")]);
    ...
    public function ticketreport(){
		...
		$data['start_date'] = (isset($_POST['start_date']))?$_POST['start_date']:date("m")."/01/".date("Y");
    	$data['end_date'] = (isset($_POST['end_date']))?$_POST['end_date']:date("m/d/Y",strtotime('last day of this month', time()));
    	$data['customer'] = (isset($_POST['customers']))?$_POST['customers']:"";
    	...
    	// get list of customers/companies
    	if($urole==1){
    		// its admin. show all companies
    		$data['customers']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', '','name','ASC');
    	}else{
    		// its employee/tech
    		if($internal_user_external_user==1){
    			// its employee
    			$where = "id IN (SELECT fk_company_id FROM customer WHERE accountmanagerid= $uid AND status=1)";
	            $data['customers']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', $where,'name','ASC');
    		}else{
    			// its tech
    			$where = "id IN (SELECT customerid FROM project WHERE FIND_IN_SET( $uid, developer ))";
	            $data['customers']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', $where,'name','ASC');
    		}
    	}
    	$data['customers']['']="All Customers";
    ...
    public function messages(){
        ...
        foreach ($details as $key=>$values){
        	...
        	if($values->from_type=='C'){
        		$details[$key]->from_email = $this->portalmodel->select_username("CONCAT(first_name,' ',last_name) as name", 'customerid', $values->created_by, 'customer');
        		...
        		if(!empty($fk_company_id)){
        			...
        			if(!empty($company_name)){
        				$details[$key]->from_email = $details[$key]->from_email." (".$company_name.")";
        			}
        		}
        	}else{
	...
	public function addmessage(){
        ...
        if ($srole==2 && $internal_external==1){
        	...
        	$to_customer = $this->portalmodel->get_dropdownuser_list('customer', 'username', "CONCAT(first_name,' ',last_name) as name", "accountmanagerid='".$id."'",'first_name','ASC');
        }else{
    ...
    public function sentmessages(){
        ...
        foreach ($details as $key=>$values){
        	//$details[$key]->from_email = $this->portalmodel->select_name('user', 'email', "id='".$values->created_by."'");
        	if($values->to_type=='C'){
        		$details[$key]->to_name = $this->portalmodel->select_username("CONCAT(first_name,' ',last_name) as name", 'customerid', $values->to, 'customer');
        		...
        		if(!empty($fk_company_id)){
        			$company_name = $this->portalmodel->select_username("name", 'id', $fk_company_id, 'customer_company');
        			if(!empty($company_name)){
        				$details[$key]->to_name = $details[$key]->to_name." (".$company_name.")";
        			}
        		}
        	}else{
    ...
    public function addinvoice(){
    	...
    	$data['customer']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', "id IN (SELECT fk_company_id FROM customer WHERE accountmanagerid='".$uid."' AND status=1)",'name','ASC');
    ...
    public function invoicedetails(){
        ...
        $where1 = "projectid IN (SELECT id FROM project WHERE customerid='".$data['details'][0]->fk_client_id."')";
        ...
        $data['customer']  = $this->portalmodel->get_dropdown_list('customer_company', 'id', 'name', "id IN (SELECT fk_company_id FROM customer WHERE accountmanagerid='".$uid."')",'name','ASC');
    ...
    public function updateticketdropdown(){
        ...
        $where = "projectid IN (SELECT id FROM project WHERE customerid='".$customer_id."')";
        $combined = "CONCAT(taskid,' - ',title) AS name";
	...
	public function categories(){
        ...
        $data['details'] = $details;
        $this->load->view('staff/categories', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function addcategory(){
        ...
        $this->load->view('staff/addcategory',$data);
        $this->load->view('templates/footer');
    }
    ...
    public function categorydetails(){
        ...
        $id                = $this->input->get('id');
        $where             = "id=$id";
        $data['details']   = $this->portalmodel->select_where('', '', 'categories', $where);
        
        $this->load->view('staff/categorydetails', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function update_category(){
        ...
        $result = $this->portalmodel->update_query('categories', $data, $id, 'id');
	...
	function company(){
		...
		$list = $this->portalmodel->select('', '', 'customer_company','','name','ASC');
        ...
        $data['list'] = $li;
        $this->load->view('staff/company', $data);
        $this->load->view('templates/footer');
    }
	...
	function addcompany(){
        ...
        $this->load->view('staff/addcompany', $data);
        $this->load->view('templates/footer');
    }
	...
	function insert_company(){
        ...
        if(!empty($existing_id) && $existing_id!='N/A'){
        	$this->session->set_flashdata('error_message', "Can not add Customer Company. Company name already exists!"); //display the flashdata using session
        	redirect("portal/addcompany");
        	exit;
        }
	...
	function companydetails(){
        ...
        $id                     = $this->input->get('id');
        $where                  = "id=$id";
        $data['details']        = $this->portalmodel->select_where('', '', 'customer_company', $where);
        
		$this->load->view('staff/companydetails', $data);
        $this->load->view('templates/footer');
    }
    ...
    function update_company(){
    	...
    	// now check if the company already exists or not.
        $companyname = $this->input->post('company');
        $existing_id = $this->portalmodel->select_name("customer_company","id","TRIM(LOWER(name))='".trim(strtolower($companyname))."' AND id!='".$id."'");
        
        if(!empty($existing_id) && $existing_id!='N/A'){
        	$this->session->set_flashdata('error_message', "Can not update Customer Company. Company name already exists!"); //display the flashdata using session
        	redirect("portal/companydetails?id=" . $id);
        	exit;
        }
        ...
        $result = $this->portalmodel->update_query('customer_company', $data, $id, 'id');
    	
        redirect("portal/company");
    }
    
	/* CRON JOBS */
    // HIGH priority ticket not assigned to any tech.
    public function check_fifteen_minutes_tickets(){
    	$this->load->model("portalmodel");
    	
    	$fifteen_minutes = date('Y-m-d H:i:s', strtotime('-15 minutes'));
    	
    	$arrEmails = array();
    	// also add support@synergyit.ca as email
		$arrEmails[] = "support@synergyit.ca";
    ...
    // Low/Medium priority tickets not assigned to any tech
	public function check_seventy_two_hours_tickets(){
    	$this->load->model("portalmodel");
	...
	// sla report
	public function responsetimegraph(){
    	$this->common();
    	...
    	if(!empty($tickets)){
    		...
    		$finaltickets[$i]['time_difference'] = 0;
    		$i++;
    	...
    	$data['customers']['']="All Customers";
    	//echo "<pre>"; print_r($data); exit;
    	$this->load->view('staff/responsetimegraph', $data);
        $this->load->view('templates/footer');
    }
    
	public function resolutiontimegraph(){
    	$this->common();
    	...
    	$data['customers']['']="All Customers";
    	//echo "<pre>"; print_r($data); exit;
    	$this->load->view('staff/resolutiontimegraph', $data);
        $this->load->view('templates/footer');
    }
