<section id="main-content">
	...
	<div class=" pull-left">
		<h3><?php /* ?><i class="fa fa-angle-right"></i><?php */ ?> Completed Workorder List</h3>
	</div>
						...
						<table class="table table-striped table-bordered table-hover" id="dataTablesWorkorders">
							<thead>
								<tr>
									...
									<th>Tech Invoice</th>
									<th>Added Info</th>
									<th style="width:110px;">Pending Info Alert</th>
									<th>Created By</th>
									<th>Created On</th>
									<th style="width:90px;">Completed On</th>
							...
							<?php if(!empty($details)){ foreach($details as $row){ ?>
								...
								<tr class="odd gradeX">
									<td><a <?php /*if($logged_in_user_id!="1001" && $row['status']==1 && $internal_user_external_user!=1){ ?>onclick="changeWorkOrderStatus(<?php echo $row['id_workorder']; ?>,2,1)"<?php }*/ ?> href="<?php echo $this->config->item('base_url')."/workorderfiles/".$row['workorder_number'].".pdf"; ?>" target="_blank"><?php echo $row['workorder_number']; ?></a></td>
									...
									<td class="center">
										<?php
											...
											if($row['status']==2){
												...
												$a_title = "Cancelled";
												$status_class = "workorder_status_cancelled";
											} 
										?>
										<span title="<?php echo $a_title; ?>" class="<?php echo $status_class; ?>" id="mark_workorder_as_done_<?php echo $row['id_workorder']; ?>" <?php /*if($row['status']==3 && $logged_in_user_id=="1001"){ ?>onclick="mark_workorder_as_done(<?php echo $row['id_workorder']; ?>)" style="cursor: pointer;" <?php }*/ ?>>
									...
									<td><?php echo $row['cb_first_name']." ".$row['cb_last_name']; ?></td>
									
									<td><?php echo date("d M, Y",strtotime($row['created_on'])); ?></td>
									
									<td><?php echo ($row['completed_on']!="0000-00-00 00:00:00")?date("d M, Y",strtotime($row['completed_on'])):""; ?></td>
									
									<td class="center">
										<span><a <?php /*if($logged_in_user_id!="1001" && $row['status']==1 && $internal_user_external_user!=1){ ?>onclick="changeWorkOrderStatus(<?php echo $row['id_workorder']; ?>,2,0)"<?php }*/ ?> href="workorderdetails?id=<?php echo $row['id_workorder']; ?>" title="Edit"><i class="fa fa-bars"></i></a></span>
									</td>
								</tr>
								<?php } } ?>
							</tbody>
